...
export class LiguesComponent implements OnInit {
  ...
  },
  {
    ...
    saveLeague(): void {
        ...
        if (this.leagueForm.get('name')?.value) formData.append('name', this.leagueForm.get('name')?.value);
        if (this.leagueForm.get('teams_count')?.value) formData.append('teams_count', this.leagueForm.get('teams_count')?.value);
        if (this.leagueForm.get('pools_count')?.value) formData.append('pools_count', this.leagueForm.get('pools_count')?.value);
        ...
        const request$ = this.isEditing && this.editingLeagueId ? this.ligueService.update(this.editingLeagueId, formData) : this.ligueService.create(formData);
    ...
    filteredTeams(): any[] {
        ...
        return this.teams.filter((team) => team.name?.toLowerCase().includes(term) || team.abbreviation?.toLowerCase().includes(term));
    }
    ...
    get filteredLeagues(): League[] {
        ...
        return this.leagues.filter((league) => league?.name?.toLowerCase().includes(this.searchTerm.toLowerCase()));
    }
