...
export class EquipeDetailsComponent implements OnInit {
    ...
    private mockTeam: Team = {
        ...
        socks_color: '#3B82F6'  */
                ...
                secondary_color: '#db0505ff',
                tertiary_color: '#FFFF00'
            },
    ...
    this.players = structuredClone(this.mockPlayers); */
        ...
        // forms
        // Get default gender from team category
        const defaultGender = this.team.category?.gender === 'MALE' ? 'MALE' : 'FEMALE';
    ...
    loadTeam(id: string) {
        ...
        this.equipeService.getById(id).subscribe({
            next: (res: any) => {
                ...
                // Initialize player form after team is loaded
    ...
    // ---------- GETTERS ----------
    get f() {
        return this.playerForm.controls as {
    ...
    get sf() {
        return this.staffForm.controls as {
            [key in keyof any]: FormControl;
        };
    }
    get kf() {
        return this.kitForm.controls as {
            [key in keyof any]: FormControl;
        };
    }
    get tf() {
        return this.trophyForm.controls as {
            [key in keyof any]: FormControl;
        };
    }
    ...
    savePlayer(): void {
        ...
        // Handle contract if provided in form
        if (v.contract_start_date || v.contract_end_date || v.contract_salary_amount) {
            // Send contract fields as array format for Laravel validation
            formData.append('contract[team_id]', this.team_id!);
            if (v.contract_start_date) {
                formData.append('contract[start_date]', new Date(v.contract_start_date).toISOString());
            }
            if (v.contract_end_date) {
                formData.append('contract[end_date]', new Date(v.contract_end_date).toISOString());
            }
            if (v.contract_salary_amount) {
                formData.append('contract[salary_amount]', v.contract_salary_amount.toString());
            }
            formData.append('contract[status]', v.contract_status || 'ACTIVE');
        } else if (this.currentPlayer?.contracts?.length) {
            // For editing players, keep existing contracts if no new contract provided
            // Transform the first existing contract to the expected format
            const existingContract = this.currentPlayer.contracts[0];
            formData.append('contract[team_id]', existingContract.team_id || this.team_id!);
            if (existingContract.start_date) {
                formData.append('contract[start_date]', existingContract.start_date);
            }
            if (existingContract.end_date) {
                formData.append('contract[end_date]', existingContract.end_date);
            }
            if (existingContract.salary_amount) {
                formData.append('contract[salary_amount]', existingContract.salary_amount.toString());
            }
            formData.append('contract[status]', existingContract.status || 'ACTIVE');
        }
    ...
    addEmergencyContact() {
        (this.playerForm.get('emergency_contact') as FormArray).push(this.fb.group({ name: [''], phone: [''], email: [''], relationship: [''] }));
    }
        ...
        this.confirm.confirm({
            ...
            accept: () => {
                switch (type) {
                    ...
                    case 'maillot':
                        this.team.kits = this.team.kits?.filter((k) => k.id !== id);
                        break;
                    case 'trophy':
                        this.team.trophies = this.team.trophies?.filter((t) => t.id !== id);
                        break;
                }
    ...
    saveTrophy() {
        if (this.trophyForm.invalid) {
            this.trophyForm.markAllAsTouched();
            return;
        }
    ...
    get filteredPlayers(): Player[] {
        return this.players.filter((p) => !this.searchPlayer || p.first_name?.toLowerCase().includes(this.searchPlayer.toLowerCase()) || p.last_name?.toLowerCase().includes(this.searchPlayer.toLowerCase()));
    }
