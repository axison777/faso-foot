...
//   teamForm = ['win', 'win', 'draw', 'loss', 'win'];
  ...
  loadTeamData() {
    ...
    this.equipeService.getTeamDashboard(userTeamId).subscribe({
      next: (t) => {
        console.log('✅ [DASHBOARD] Équipe reçue du backend:', t);
        // console.log('✅ [DASHBOARD] Équipe reçue du backend:', t.nextMatch);
        this.team.set(t);
        ...
        // console.log('🔄 [DASHBOARD] Chargement du prochain match', data.nextMatch);
        this.topScorers.set(data.topScorers);
        this.topAssisters.set(data.topAssisters);
        this.teamForm.set(data.teamForm);
        this.loadNextMatch(data.nextMatch);
      },
      error: (err) => {
  ...
  loadNextMatch(nextMatch: any){
    console.log('🔄 [DASHBOARD] Prochain match trouvé:', nextMatch);
   const formatted = {
        ...nextMatch,
        ...
        stadium: nextMatch.stadium?.name || 'Stade',
        isHome: nextMatch.isHome,
        homeTeam: nextMatch.homeTeam?.name || nextMatch.homeTeam?.abbreviation || 'Équipe Domicile',
        awayTeam: nextMatch.awayTeam?.name || nextMatch.awayTeam?.abbreviation || 'Équipe Extérieure',
...
//   }
  ...
  prepareMatchSheet() {
    ...
    // console.log('📋 [DASHBOARD] Match trouvé:', match);
    if (match && match.id) {
